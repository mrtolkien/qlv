...
module.exports = {
  ...
  body: (p) => `
    ...
    (module ChocBonusGoodies (layer F.Cu) (tedit 56D1B4CB)
        ...
        ${maybe(
          () => hotswap_silk_screen("", "-", "F"),
          p.hotswap_silk_screen && p.reverse
        )}
...
const hotswap_plate_cutout = () => {
  ...
  const mirror = 1; // or -1
  const points = [
    { x: -4.575, y: -7.25, dx: -1, dy: -1 }, // pad1 tl
    { x: -2.0, y: -7.25, dx: -8, dy: -1 }, // pad1 tr
    { x: -2.0, y: -7.7, dx: -8, dy: -3 }, // pad1
    { x: -1.5, y: -8.2, dx: -8, dy: -3 }, // pin1 top
    ...
    { x: -2.0, y: -4.2, dx: -8, dy: 3 }, //
    { x: -2.0, y: -4.65, dx: -8, dy: 1 }, // pad1 br
...
const vias = (p) => {
  const { def_neg, def_pos } = p.reverse
    ? { def_neg: "", def_pos: "-" }
    : { def_neg: "-", def_pos: "" };
